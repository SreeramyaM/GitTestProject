...
public class BestInetPrivilegedUsersDetailsPageProp {
	
	private static final Logger logger = LogManager.getLogger(BestInetPrivilegedUsersDetailsPageProp.class.getName());
	...
	static{
		try {
			if(prop.isEmpty()){
				logger.info("Loading BestInetPrivilegedUsersDetailsPage.properties");
				String lang = "en";
				if(configProp.containsKey("language")){
					lang = configProp.getProperty("language");
				}
				prop.load(BestInetPrivilegedUsersDetailsPageProp.class.getResourceAsStream("/language/"+lang+"/cdc/BestInetPrivilegedUsersDetailsPage.properties"));
			}
