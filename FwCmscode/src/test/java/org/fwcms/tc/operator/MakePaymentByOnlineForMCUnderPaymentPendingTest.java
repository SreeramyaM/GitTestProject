...
/**MC_Payment_toBestinet_27 : Confirm the functionality of the "Submit" button on the online transfer details form.
 * MC_Payment_toBestinet_28 : Verify the functionality of the change of status from 'pending realization' to 'realized' in case of online transfer 
 * MC_Payment_toBestinet_29 : Verify the contents in the window when the functionality of the change of status from 'pending realization' to 'realized' in case of online transfer is checked.*/


public class MakePaymentByOnlineForMCUnderPaymentPendingTest extends BrowserInitiator {
private static final Logger logger = LogManager.getLogger(MakePaymentByOnlineForMCUnderPaymentPendingTest.class.getName());
	...
	public Object[][] returnMedicalCenterRegistration() throws Exception{
		JSONArray ja = (JSONArray) new JSONParser().parse(new FileReader(MakePaymentByOnlineForMCUnderPaymentPendingTest.class.getResource("/org/fwcms/tc/mcRegistration/MedicalCenterRegistration.json").getFile()));
	...
	public void makePaymentByOnlineForMCUnderPendingPaymentStatus(HashMap<String, String> data) throws Exception{
		...
		String TransactionID=RandomStringUtils.randomNumeric(4);
		...
		MakePayment.payAmountThorughOnlineTransfer(pendingAmount,TransactionID, "tester", "tester", "Online Transfer");
		...
		MakePayment.verifyingTransactionIDInPaymentHistoryTable(TransactionID);
		MakePayment.changingPendingRealisedtoReaslised(TransactionID);
		MakePayment.verifyConfirmationPopupMessageAnClickOK("Do you want to update the status of transaction as REALIZED ?");
		new CommonElements(driver).initElements().waitForStatusBarToBeVisibleAndVerifyTextAndCloseStatusBarAndWaitForOverlayToHIde("Transaction captured successfully");
