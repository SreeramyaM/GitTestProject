...
/**MedCentre_Verify_34 : Verify the functionality of the "Reject" button. 
 * MedCentre_Verify_37 : Verify free navigation of the representative among the different tabs.*/


public class VerifyRejectMCFunctionalityTest extends BrowserInitiator {
private static final Logger logger = LogManager.getLogger(VerifyRejectMCFunctionalityTest.class.getName());
	...
	public Object[][] returnMedicalCenterRegistration() throws Exception{
		JSONArray ja = (JSONArray) new JSONParser().parse(new FileReader(VerifyRejectMCFunctionalityTest.class.getResource("/org/fwcms/tc/mcRegistration/MedicalCenterRegistration.json").getFile()));
	...
	public void verifyRejectMCFunctionality(HashMap<String, String> data) throws Exception{
		...
		SearchMedicalCenterPage rejectMC  = new SearchMedicalCenterPage(driver).initElements();
		rejectMC.searchByNameAndStatus(data.get("medicalcentername"), "Pending Verification");
		new CommonElements(driver).initElements().waitForOverlayToHide();
		rejectMC.clickApplicationId();
		rejectMC.VerifyFreeNavigationOfMCTabs(); 
		rejectMC.rejectMedicalCenter();
		rejectMC.rejectConfirmationPopupWithReason("Reject MC");
		new CommonElements(driver).initElements().waitForStatusBarToBeVisibleAndVerifyTextAndCloseStatusBarAndWaitForOverlayToHIde("Application Rejected!\n\nMail is sent to medical center users with Application Rejected status.");
	}
}