...
/**MedCentre_Verify_21 : Check for the Reject button feature*/

public class RejectMCFuntionalityTest extends BrowserInitiator {
private static final Logger logger = LogManager.getLogger(RejectMCFuntionalityTest.class.getName());
	...
	public Object[][] returnMedicalCenterRegistration() throws Exception{
		JSONArray ja = (JSONArray) new JSONParser().parse(new FileReader(RejectMCFuntionalityTest.class.getResource("/org/fwcms/tc/mcRegistration/MedicalCenterRegistration.json").getFile()));
	...
	public void rejectMCFuntionalityTestAsPrivilegedUser(HashMap<String, String> data) throws Exception{
		...
		new CommonElements(driver).initElements().waitForOverlayToHide();
		smcp.rejectMCButton();
		smcp.rejectConfirmationPopupWithReason("Rejected");
		smcp.verifySuccessMessage("Medical center is rejected");
	}
}
